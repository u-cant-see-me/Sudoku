    ...
    },
    {
      ...
      icon: (
        <>
          <FaBrain />
          <span
            className="absolute top-0 right-0 translate-x-[40%] -translate-y-[20%]
    ...
    <div className="flex flex-wrap sm:flex-nowrap gap-4 md:block xl:flex">
      <div className=" flex gap-4 mb-2">
        {buttons.map((btn, index) => (
          <button className={btn.css} type="button" title={btn.title} key={index} onClick={btn.onClick}>
            {btn.icon}
          </button>
        ))}
      </div>
      <div className="flex gap-4 mb-2">
        ...
        <Timer state={state} setState={setState} resetSignal={resetSignal} />
      </div>
    </div>
  );
};
